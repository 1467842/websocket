...
(function ($, window) {
    ...
    Cityselect.defaults = {
        ...
        hotCity: ["北京","天津","沈阳","大连","上海","南京","苏州","杭州","青岛","郑州","武汉","长沙","广州","深圳","重庆","成都","西安","香港","澳门"],
        onInit: function () {},
        onForbid: function () {},
        ...
        $.each(configure.contain, function (key, value) {
            ...
            if (parameter.options.hotCity.length < 1 && key < 18) {
                ...
                $.each(parameter.options.hotCity, function (hkey, hvaluef) {
                    let v = `${hvaluef}市`;
                    let t = value.name.substring(value.name.length - 1,value.name.length);
                    if(t === "市") {
                        if (v == value.name) {
                            parameter.filterCity.HOT.push(value);
                        }                        
    ...
    functionality.template = [
        '<div class="city-pavilion hide" style="margin-top:0px;border-radius:6px;">',
            '<div class="city-tabs">',
            ...
            '</p>',
        '</div>',
        '<div class="city-info{type}" style="border-radius: 6px">',
            '{name}',
            '<div class="city-input {not}" style="display:none;">',
    ...
    functionality.showDrop = function (event) {
        ...
        if(typeof self.$selector[0].classList[1] == "undefined") {
            self.$selector.addClass('down').find('.city-pavilion').removeClass('hide').siblings('.city-list').addClass('hide');
        } else {
            var $citySelect = $('.city-select');
            $citySelect.removeClass("down");
            $citySelect.find('.city-pavilion').addClass("hide");
    ...
    functionality.singleAchieve = function (event) {
        ...
        this.$selector.find('.city-input').before('<span data-id="'+ id +'" data-parentid="'+ parentId +'">'+ name +'</span>').find('.input-search').val('');//<i class="del"></i>
    ...
    functionality.multiAchieve = function (event) {
        ...
        if (!hasActive) {
            ...
            if ($.inArray(name, self.multiSelectResult) < 0) {
                ...
                //拼接生成选中值
                joinSpan = '<span data-id="'+ id +'" data-parentid="'+ parentId +'">'+ name +'</span>';//<i class="del"></i>
            ...
            for (var i = 0; i < cityArray.length; i++) {
                ...
                $.each(self.newCityData, function (key, value) {
                    ...
                    if (cityArray[i] === value.name) {
                        ...
                        //选中的值拼接dom
                        joinSpan = '<span data-id="'+ value.id +'" data-parentid="'+ value.parentId +'">'+ name +'</span>';//<i class="del"></i>
